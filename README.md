...
cp todo-map.conf.example todo-map.conf      # optionnel
```
...
| Règle | Effet |
|-------|-------|
| Pas de supposition | interdit de coder contre une forme de payload / schéma non observé dans le code ou une réponse réelle |
...
- Contexte de session : `context-update` (reprise sans argument, init/maj avec), `checkpoint`, `summarize-session`, `clean-context`
...
declare -A TODO_MAP=(
  ["mon_projet"]="mon-projet"
  ["mon_monorepo"]="mon-projet autre-tag"
)
```

Le tag est cherché **en entier**, jamais en préfixe : un dossier sans entrée fait dire aux
deux consommateurs que le toDo projet n'a pas été chargé, au lieu de rendre une liste vide
qui se lirait « rien à faire ».
